...
\begin{equation}
\begin{split}
\Pro(f \in t, \mathrm{pos}) = \frac{\alpha_t}{\tildel{t}}
\end{split}
(\#eq:3)
\end{equation}
...
\begin{equation}
\begin{split}
\Pro(f \in t_k|\mathrm{pos}, \alpha^{(n)}) &= \frac{\Pro(f \in t_k, \mathrm{pos}|\alpha^{(n)})}{\sum\limits_{k=1}^{K} \Pro(f \in t_k, \mathrm{pos}|\alpha^{(n)})} \\
&= \frac{\alpha_k^{(n)} \frac{y_k}{\tildel{k}}}{\sum\limits_{k=1}^{K} \alpha_k^{(n)} \frac{y_k}{\tildel{k}}} \\
&= \lambda_k^{(n)}
\end{split}
(\#eq:5)
\end{equation}
...
\right) \\
&= \argmax_\alpha \left( 
\sum_{f \in F} \sum_{k=1}^{K} \lambda_k^{(n)} \log\left(
\alpha_k^{(n)} \frac{y_k}{\tildel{k}}
\right)
\right) \\
&= \argmax_\alpha \left(
\sum_{f \in F} \sum_{k=1}^{K}  \lambda_k^{(n)} \log\left(
...
\begin{equation*}
\begin{split}
\alpha_k^{(n+1)} &= \frac{\sum\limits_{f \in F} \lambda_k^{(n)}}{\sum\limits_{i=1}^{K} \sum\limits_{f \in F} \lambda_i^{(n)}} \\
...
\begin{equation}
\begin{split}
\Pro(f \in r_u) = \frac{\rho_u \tildel{u}}{\sum\limits_{u \in U} \rho_u \tildel{u}} = \alpha_u \\
...
\begin{equation}
\begin{split}
\Pro(f \in s_v) = \frac{\theta_v \tildel{v}}{\sum\limits_{v \in V} \theta_v \tildel{v}} = \beta_v \\
